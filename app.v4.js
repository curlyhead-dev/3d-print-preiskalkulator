...
function normalizeState(input) {
  ...
  const numericKeys = [
    "materialEurPerKg", "weightG", "materialLossPct", "printHours", "printMinutes", "printTotalMinutes",
    "machineEurPerHour", "electricityEurPerKwh", "printerPowerW", "setupCostEur", "overheadPct",
    "profitMarginPct", "vatPct", "roundingStep", "designHours"
  ];

  for (const k of numericKeys) {
    const n = Number(merged[k]);
    merged[k] = Number.isFinite(n) ? n : DEFAULTS[k];
  }

  merged.quantity = Math.max(1, Math.floor(Number(merged.quantity) || DEFAULTS.quantity));
...
function applyState(state) {
  ...
  fields.projectName.value = s.projectName;
  fields.materialPreset.value = s.materialPreset;
  fields.materialEurPerKg.value = s.materialEurPerKg;
  fields.weightG.value = s.weightG;
  fields.materialLossPct.value = s.materialLossPct;
  fields.printHours.value = s.printHours;
  fields.printMinutes.value = s.printMinutes;
  fields.printTotalMinutes.value = s.printTotalMinutes;
  fields.machineEurPerHour.value = s.machineEurPerHour;
  fields.electricityEurPerKwh.value = s.electricityEurPerKwh;
  fields.printerPowerW.value = s.printerPowerW;
  fields.setupCostEur.value = s.setupCostEur;
  fields.overheadPct.value = s.overheadPct;
  fields.profitMarginPct.value = s.profitMarginPct;
  fields.vatPct.value = s.vatPct;
  fields.roundingStep.value = s.roundingStep;
  fields.designHours.value = s.designHours;
  fields.quantity.value = s.quantity;
}
